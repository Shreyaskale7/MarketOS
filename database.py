...
class DailyInsight(Base):
    ...
    # The forward/opportunity narrative was being generated by the pipeline
    # and written to the daily JSON, but there was no column for it here --
    # so it never reached the database and the dashboard's "Forecast
    # Intelligence & Macro Triggers" panel had nothing to read, permanently
    # showing "No forward insight for today".
    forward_insight = Column(Text)
    regime_label   = Column(String(50))
...
def _migrate_add_column(table: str, column: str, ddl_type: str):
    """create_all(checkfirst=True) never adds columns to an existing table."""
    from sqlalchemy import inspect as _inspect, text as _text
...
def ensure_tables_exist():
    ...
    with _init_lock:
        if not _tables_created:
            ...
            try:
                _migrate_add_column("daily_insights", "forward_insight", "TEXT")
            except Exception as exc:
                print(f"  [migration] daily_insights migration skipped: {exc}")
